...
```
Another [Router type is the](https://reactrouter.com/en/main/router-components/hash-router) `[HashRouter](https://reactrouter.com/en/main/router-components/hash-router)`. While its usage is similar to the `BrowserRouter`, it stores the current location in the hash portion of the URL. The URL we get with this Router is not so clean like so:
