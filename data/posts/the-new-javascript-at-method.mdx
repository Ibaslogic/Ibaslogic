...
publishedAt: '2021-11-23 10:32:00'
updatedAt: '2021-11-23 10:32:00'
featured: '/static/images/the-new-javascript-at-method/banner.png'
external_post: true
external_url: 'https://blog.logrocket.com/using-new-javascript-at-method/'
category: 'JavaScript'
author_name: 'Megan Lee'
author_twitter: '@megan'
author_link: 'https://twitter.com/megan'
author_avatar: '/megan.jpg'
toc: true;
---
...
In this tutorial, we’ll look at the JavaScript `.at()` method, its use cases, and how it can improve developers’ experiences compared to the preexisting methods.

## The indexable object prototype

The ` .``at() ` method is located on the `prototype` of indexable objects.
...
console.log(
  appendNumber([1, 2, 'three', 4, 5, true, false], 6).at(-1)
);
// Expected Output: 6
```

In the code, the `.at()` method applies directly on the returned value without first storing the value in a variable.
...
With `Math.random()`, we get a floating-point number between `0` (inclusive) and the array length. Then, when the `.at()` method receives this floating-point number as an index, it considers the value before the decimal point and returns the element at that index.
...
At the time of writing this tutorial, the `.at()` method works pretty well with the popular modern browsers, except a few. See [browser support here](https://caniuse.com/mdn-javascript_builtins_array_at).
