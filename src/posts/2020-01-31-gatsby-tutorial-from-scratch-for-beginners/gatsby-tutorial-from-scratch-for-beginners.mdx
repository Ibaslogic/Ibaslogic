...
category: "Gatsby"
datePublished: "2020-01-31 10:32:00"
dateUpdated: "2020-01-31 10:32:00"
...
Otherwise, you can source your content from CMSs like Contentful, WordPress or Drupal. This way, you can build a static website powered by Gatsby but still manage your content through a nice admin interface.

Either way, there are Gatsby plugins that allows us to achieve any of these.
...
From the computer terminal, navigate into your site's directory:
...
Feel free to use any image on your hard drive. Or [check my GitHub repo](https://github.com/Ibaslogic/ibaslogic-gatsby-tutorial/tree/master/src/images "download project image") to download the same header image used in this project.
...
At the moment, we are only rendering the excerpt of our markdown content on the blog page. Later, we will get to display the featured image as we have it in the finished product.
...
The first step is only applicable if you are sourcing your content from the markdown files. For CMSs like Contentful, you don’t need to create slugs yourself. 
...
In the code, we are destructuring the `createPages` API function and grabbing the `graphql` function which allows us to fetch the markdown slugs we created earlier.

This `graphql` function returns a promise and so we used the `async/await` syntax to get the promise response.

Since this response holds all of the data we need including the slug, we looped through its `edges` array (which contains the list of posts) and run the `createPage` action for each of them.

This `createPage` action is always called to generate a page.

It accepts an object where we specified the `path` for visiting the single blog post, the `component` parameter allows us to specify the absolute path to the template file while the `context` contains the data (i.e slug) we are passing to the template file.
...
To see how this is done, let’s play around with the GraphiQL IDE.
...
In the code, you will notice we are not using the `useStaticQuery` hook for our query as we normally do. This is because the static query cannot take query variables.
...
Instead of using the name given to you by netlify, you can change it to something that you can easily read. To do that, click on the **Settings** tab and then click on the **Change site name** button to specify a name.
