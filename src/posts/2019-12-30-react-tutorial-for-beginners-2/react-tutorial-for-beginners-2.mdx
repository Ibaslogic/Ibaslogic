---
title: "React Tutorial: Form handling, Raising Events and the setState method"
description: "In React, form handling works a little bit different unlike HTML form that is handled by the DOM"
category: "React"
datePublished: "2019-12-30 10:29:00"
dateUpdated: "2019-12-30 10:29:00"
featured: "./images/react-tutorial-for-beginners-part-2.png"
tags: ["react", "reactjs"] 
--- 


In the first part of this React tutorial, we covered some of the basic React fundamentals, set up React working environment and started writing our React app.
...
Now that you know some of the React basic concept, it's time to dive deeper and explain the React form handling and handling event.
...
At the moment, this is the current status of our app:

![React devtools](./images/react-devtools.png)

Ok. Let’s add an `input` tag of type `checkbox` inside of the `li` element – just before `{this.props.todo.title}` – in the `TodoItem` component.
...
By default, the `input` type (in this case, checkboxes) are being handled by the DOM – i.e they have the default HTML behaviour. That is why you can toggle the boxes. This type of input is called **uncontrolled input**.

In React, the input fields are meant to be **controllable**. 
 
What does this mean and how do we go about it? First, we need to understand what controlled component is. 

### Controlled Component

To make the input fields controllable, the input data (in this case, toggling of checkboxes) has to be handled by the component state and not the browser DOM. With this, the state will serve as a **single source of truth**.

This is necessary because the state will not change unless you change it.
...
At this point, you have succeeded in making these input checkboxes a controlled input because it no longer listens to its internal state but the state in your application.

Now if you try to toggle any of the checkboxes, nothing will happen. This is because each of the checked attributes is assigned a value equal to the current value of what the state is.

As we have it in the state, only the first task is completed.
...
Open the console. You'll see a warning displayed as a result of the added `checked` attribute.
...
Instead of logging text in the console, we want to toggle the checkboxes anytime they are being clicked. To do this, we need to understand how to raise and handle events.
...
So update the `TodosList` instance, `<TodosList />` so you have:
...
Now, you have the `handleChange()` data assigned to the `handleChangeProps`. This data can be accessed through props in the `TodosList` component. From there, we can pass it to the `TodoItem` component.
...
This method is called `setState()`. It tells React that we are updating the state. Then React figures out what part of the state is changed and then update ONLY that part in the real DOM.
...
The `id` on the first line comes from the `TodoItem` component (it contains the checked id). This id is then compared to each of the todos items id. If any of the id matches, then we flip the `completed` value.
...
The way we will do this will be similar to how we handled the input checkbox. Since the todos items live in the `TodoContainer`, while the delete button will live in the `TodoItem` component, we will need to raise an event from the `TodoItem` component and move up levels until we get to the `TodoContainer` component where the event will be handled.
...
At the moment, we are logging "deleted" text alongside the ids of the deleted items.
...
Just like the `map()` method, the `filter()` is also an higher-order function. It returns a new array by applying a condition on every array element.
...
Since we will be getting data through the user’s interaction (i.e through the input field), this component will, therefore, hold state. For that reason, it has to be a class-based component.
...
Then, add its instance, `<InputTodo />` inside the `render()` method just below the `<Header />`.
...
Save the file. You should have the form field rendered in the frontend.

As we did for the checkbox, we have to make the form input field a controlled field.

The first step is to have the state manage our data. So, add this code just above the `render()` method in the `InputTodo` component:
...
> **Note:** We used `checked` props for the input checkbox and `value` props for the text input.

Now, the text input field is being controlled by the component state and not the DOM. You will not be able to write anything in the field because it is assigned a value equal to the current value of what the state is.
...
Remember we are using `this.onChange` because the method, `onChange()` is part of the class.
...
At this point, you should know that when you type inside the input field, the `onChange` event handler will trigger. This will then call the `onChange()` class method that will re-render the state using the `setState()` method.
...
But this time, we will raise an event from the `InputTodo` component (that accept the user’s input) to the parent component, `TodoContainer` where the state data is to be updated.
...
The ids help React to identify which item is added or removed.
...
If something wasn't clear, do let us know. And if you found an error somewhere in this article, you can [edit on GitHub](https://github.com/Ibaslogic/Ibaslogic/blob/master/src/posts/2019-12-30-react-tutorial-for-beginners-2/react-tutorial-for-beginners-2.mdx "Ibaslogic GitHub").
