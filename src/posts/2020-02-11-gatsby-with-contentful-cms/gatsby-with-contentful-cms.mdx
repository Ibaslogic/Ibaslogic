...
When it comes to page creation in Gatsby, the `src/pages` directory is where it looks to create site pages. Files in this directory automatically become static pages with paths based on their names.
...
All that is required to load content in the frontend is the Content Delivery API from the CMS.
...
Once this is done, you can create an empty space. Think of this space as a storage area for your project content.
...
Make sure you select **Create an empty space** and then proceed.
...
Let’s click on the **Content model** tab to add our content type.
...
For our Blog Post model, we will be using four of these fields. The **Text** for the post title, slug and excerpt, **Date and time** for the date, **Media** for the featured image and **Rich text** for the post body.

![Content type](./images/content-type.png)

To add a field for the Title, select **Text** and give the field the name of Title. Then click **Create**.
...
The code should be self-explanatory. It is similar to how you’d render the markdown data.
...
So, we need to generate a single post page.
...
Also, we will need a template file alongside the `gatsby-node.js`. This template file will render every single post on the screen.
...
We are making use of an API function, `createPages` to dynamically generate a single post page. This function accepts two parameters, `graphql` and an `actions` object.

The `graphql` function which allows us to fetch the post slugs from Contentful CMS returns a promise. And so we used the `async/await` syntax to get the promise response.

Since this response holds all of the data we need including the `slug`, we looped through its `edges` array (which contains the list of posts) and then dynamically create the pages for each of them by calling the `createPage` function.

In this function, we specified the parameters needed to create the pages.

Notice we are referencing the template file that we need to create in the `src` directory.
