...
In the code, we are making use of the `createPages` API function to dynamically create a single post page. This API accepts two parameters, `graphql` and an `actions` object.

The `graphql` function which allows us to fetch the markdown `slugs` we created earlier returns a promise. And so we used the `async/await` syntax to get the response.

Since this response holds all of the data we need, we looped through its edges array (which contains the list of posts) and then dynamically create the pages for each of them by calling the `createPage` function.

This function accepts an object where we specified the `path` for visiting the single blog post, the `component` parameter which allows us to specify the absolute path to the template file while the `context` contains the data (i.e `slug`) we are passing to the template file.
