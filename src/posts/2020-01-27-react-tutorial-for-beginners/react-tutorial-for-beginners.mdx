...
So learning React is necessary if you want to be a present-day developer. 
...
Now, if the state of this element changes, React would recreate the virtual DOM for the second time and compare with the previous version to detect which virtual DOM object has changed.
...
Once you have it installed, run the following command from your terminal: 
...
> **Note:** The `import` statement is an ES6 feature that allows us to bring in objects (`React` and `ReactDOM`) from their respective modules (`react` and `react-dom`). A **module** is just a file that usually contains a class or library of functions. 
> 
> And `create-react-app` CLI has both files installed for us to use. 

Notice also, we are not loading Babel to compile JSX to JavaScript. It comes bundled with the CLI - and it uses it under the hood.
...
Well, this has been solved as we can now create React components ONLY with function. Thanks to React Hooks.
