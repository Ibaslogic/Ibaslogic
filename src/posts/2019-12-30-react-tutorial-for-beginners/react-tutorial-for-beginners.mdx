---
title: "React Tutorial: The Beginner's Guide to Learning React in 2020"
description: "In this React tutorial, you will learn the React basic concept. You will also learn how to create and deploy React application on the web."
category: "React"
datePublished: "2019-12-30 10:27:00"
dateUpdated: "2019-12-30 10:27:00"
featured: "./images/react-tutorial-for-beginners.png"
tags: ["react", "reactjs"]
--- 

Welcome to your [React learning](https://reactjs.org/ "React documentation") journey!

Are you familiar with HTML, CSS and JavaScript? Then, you can build a simple and dynamic website.

But have you taught about building modern web applications?

In this era, modern websites are embracing the Single Page Application (SPA) design approach whereby a user’s interaction immediately reflect in the user interface (UI) and does not require a page reload. 

With React, creating this type of application is a piece of cake.

In this React tutorial, you will learn the React basic concept. You will also learn how to create and deploy React application on the web.

But before you go ahead, please make sure you have:
...
Before we dive in, here is the React application we will build - [Simple React App](https://ibaslogic.github.io/simple-todo-app/ "simple react app"). And you can find all of the code in my GitHub repo [here](https://github.com/Ibaslogic/simple-todo-app "simple react app GitHub repo").
...
I mentioned that it allow developers to create complex UI from an isolated snippet of code. To understand this statement better, you need to start thinking in React Component.
...
We will create this app from scratch in this tutorial. It may look simple in the eye but trust me, you will get to understand the concept of React and how it works after you build this app.
...
Having this instinct indicate that you are *thinking in React!*
...
React makes use of Virtual DOM which uses a strategy that updates the DOM without having to redraw all the webpage elements. It ensures that the real DOM receive only the necessary data to repaint the UI.

This is how it works,

Whenever new elements are added to the UI, a virtual DOM is created. But if the state of any of these elements changes, React would update the virtual DOM while still maintaining the previous version so that it can compare and figure out which virtual DOM objects have changed. React then update ONLY those objects on the real DOM thereby reduces the performance cost of re-rendering the webpage.

DO not worry if all these seem strange, you will get to see them in practice later.

### Setting up Working Environment

There are several ways we can interact and get started with React. Though React recommends setting up through the `create-react-app` CLI tool, I will quickly walk you through how to start working with React by simply writing React code in HTML file.
...
The first argument of the `render()` method defines what you want to render while the second defines where you want to render it. As seen in the code, we used plain vanilla JavaScript to reference the `div` container (through the `root` id) in our `.html` file.
...
Once you have it installed, run the following command in your terminal to create a React App called `simple-todo-app`.
...
Head over to your terminal and run this command:

```
C:\Users\Your Name\simple-todo-app > npm start
```

> Please make sure you are in the project directory before you run the command. Also, if you are on VsCode, you can toggle the terminal by pressing ``Ctrl + ` `` or ``Cmd + ` `` or go to `View-> Terminal`.
...
If nothing happens, write this URL, `localhost:3000` in the browser address bar and press enter.
...
At this point, we are good to start creating our React App. You will learn how to create separate components that describes how different sections of the UI looks like. These components files live in the `src` folder.
...
Comparing this code to the one we write directly in the HTML file at the beginning. You'll see that we didn’t do anything special except that we imported `React` and `ReactDOM` instead of loading their respective CDN.

> **Note:** The `import` statement is an ES6 feature that allows us to bring in objects (`React` and `ReactDOM`) from their respective modules (`react` and `react-dom`). A **module** is just a file that usually contains a class or library of functions. And `create-react-app` has both files installed for us to use. 

Notice also, we are not loading Babel to compile JSX to JavaScript as `create-react-app` have it installed and uses it under the hood.
...
The `TodoContainer` component which is the parent component was not connected to the app until we imported and rendered it in the `index.js` file.
...
This is necessary so that its instance (e.g `<TodoContainer />`) in JSX is not considered as HTML tag.
...
To add a state, we simply create a state object with different key-value pair. For the todos data, we will have an array of objects.

In the `TodoContainer.js` file, add the following code just above the `render()` method:
...
At the moment, you’ll see React warnings if you check the console of your devtools. We will take care of that in a moment.
...
> **Note:** Since we are mapping through the todos, don’t forget to add `key` props. 

At this point, each of the state data is present in the `todo` props. You can now access these data through props in the `TodoItem` component.
...
Up to this moment, we have been using the **class-based** component. This type is required if the component will hold the state data and/or lifecycle method (more on this later). This is why the class component is also called a **stateful component**.

On the other hand, the **functional component** also called a **stateless component** doesn’t require the lifecycle method and cannot maintain the state data unless it uses something called React hooks. This type is the simplest form of React component because it is primarily concerned with how things look.

If you take a look at the components we’ve created, only one of them is holding the state data. And that is the parent component, `TodoContainer`. 

That means this component can only be class-based. The other components which are presently class components can also be functional-based. This is because they do not hold state data.

For clarity, let’s convert one of the class component, `TodoItem`, to a functional component.
...
As you start with React, you may not always know whether to use a functional or class component. A lot of times, you will realize after a while that you chose the wrong type. But as you create more components, making this choice will get easier.
...
In this tutorial, we will use the functional component type simply for presentation. As in the case of the `Header` component where we will focus on rendering simple heading text. So, revert the `TodoItem` component to class component.
...
Notice how we used the ES6 arrow function:
...
Lastly, if something wasn't clear, endeavour to let us know. And if you found an error somewhere in this article, you can [edit on GitHub](https://github.com/Ibaslogic/Ibaslogic/blob/master/src/posts/2019-12-30-react-tutorial-for-beginners/react-tutorial-for-beginners.mdx "Ibaslogic GitHub").  
