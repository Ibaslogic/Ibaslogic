...
In the first part of this React series, we covered some of the basic React fundamentals, set up React working environment and started writing our React app.
...
Let’s start by adding the input `checkbox` inside of the `li` element. 

So add this just before `{this.props.todo.title}` in the `TodoItem` component:
...
To make the input field controllable, the input data (in this case, toggling of checkbox) has to be handled by the component state and not the browser DOM. 

With this, the state will serve as a **single source of truth**. What this means is that the input checkbox would no longer listens to its internal state (i.e the browser DOM) but the state in your app.  
