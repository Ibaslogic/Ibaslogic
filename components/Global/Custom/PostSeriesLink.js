...
const PostSeriesLink = ({ label, datas }) => {
  return (
    <div className="my-6 p-5 border border-solid border-[#eaeaea]">
      <p className="!text-lg md:!text-xl leading-5 font-semibold mb-5 !mt-0">
        {label}
      </p>
      <ul className="flex flex-row flex-wrap justify-between list-none leading-6 !mb-0 pl-0">
        {datas.map((data, index) => (
          ...
          >
            ...
            <span className="flex-[7.8] md:flex-[8]">
              <Link
                href={data.url}
                className="text-inherit leading-5 no-underline border-none hover:border-solid hover:border-b hover:border-[#000]"
              >
                {data.label}
              </Link>
            </span>
